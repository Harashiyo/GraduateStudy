...
for m in range(4):
	f = open("C:\\Users\\Shohei\\Documents\\sotsuken\\result3\\"+getMarkName(m)+".txt","w")
	for l in range(4):
		for k in range(7):
			files = os.listdir(getmark3Dir(k))
			if "Thumbs.db" in files: files.remove("Thumbs.db")	
			cascade = cv2.CascadeClassifier(getCascadeName(l,getMarkName(m)))
			f.write(getmark3Dir(k).replace("C:\\Users\\Shohei\\Documents\\sotsuken\\mark3\\","")+" cascade"+ getDirNum(l)+"\n")
